...
@testset "BusinessDaily" begin
    ...
    # when :holidays=true and the mapped holidays and NaNs are on the same days, the values returned are the same but with the NaNs/imputed values removed.
    ...
    # when :holidays=false and the mapped holidasy and NaNs are on the same days, some valid values are removed when shifting/lagging the TSeries.
    ...
    ## Note that the Canadian holidays maps are currently wrong! It sets, Dec 24, 27 and 31 as holidays. The real values are 27 and 28
    TimeSeriesEcon.set_holidays_map("CA", "ON");
    @test values(tsbd[end-9:end]) ≈         [1.38, 1.44, 1.42, 1.44, NaN, 1.47, 1.45] nans = true
    @test values(tsbd_shifted[end-9:end]) ≈ [1.38, 1.44, 1.42, 1.44, 1.46, 1.47, 1.45, 1.42] nans = true
    @test values(tsbd_lagged[end-9:end]) ≈  [1.38, 1.44, 1.42, 1.46, 1.46, 1.47, 1.42] nans = true
    @test values(tsbd_diffed[end-9:end]) ≈  [0.06, 0.06, -0.02, 0.02, NaN, 0.01, -0.02] nans = true
    @test values(tsbd_pct[end-9:end]) ≈     [4.55, 4.35, -1.39, 1.41, NaN, 0.68, -1.36] nans = true atol=0.01
