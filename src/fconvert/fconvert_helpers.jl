...
# function _fconvert_start_overhang(F_to::Type{<:Union{<:YPFrequency}}, F_from::Type{<:Union{<:YPFrequency}}; check_parameters_from=false, check_parameters_to=false)
#     # start_values_base = trim ∈ (:begin, :both) ? :end : :begin
#     adj_from = 0
#     if check_parameters_from
#         adj_from = 12 - getproperty(F_from, :parameters)[1]
#     end
#     base_month_to = 12 / ppy(F_to)
#     if check_parameters_to
#         base_month_to = getproperty(F_to, :parameters)[1]
#     end
    
#     # alignment of start and end months
#     (d,r) = divrem(adj_from - base_month_to, 12/ppy(F_from))
#     println(adj_from, ", ", base_month_to,  ", ", 12/ppy(F_from))
#     return r
# end

# having these different signatures significantly speeds up the performance; from a few microseconds to a few nanoseconds
# get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:YPFrequency}) = _fconvert_start_overhang(F_to, F_from)
# get_overhang_to_lower(F_to::Type{<:Union{Quarterly{N}, Yearly{N}}}, F_from::Type{<:YPFrequency}) where N = _fconvert_start_overhang(F_to, F_from, check_parameters_to=true)
# get_overhang_to_lower(F_to::Type{<:Union{Quarterly{N1}, Yearly{N1}}}, F_from::Type{<:Union{Quarterly{N2}, Yearly{N2}}}) where {N1,N2} = _fconvert_start_overhang(F_to, F_from, check_parameters_to=true, check_parameters_from=true)
# get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:Union{Quarterly{N}, Yearly{N}}}) where N = _fconvert_start_overhang(F_to, F_from, check_parameters_from=true)
# get_overhang_to_lower(F_to::Type{<:Union{Yearly{N1},Quarterly{N1}}}, F_from::Type{<:YPFrequency}) where N1 = _fconvert_start_overhang(F_to, F_from)

# get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:YPFrequency}) = _fconvert_start_overhang(F_to, F_from)
# # get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:Union{Yearly{N},Quarterly{N}}}) where N = _fconvert_start_overhang(F_to, F_from)
# get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:Union{Yearly,Monthly,Quarterly}}) = _fconvert_start_overhang(F_to, F_from, check_parameters_from=false)
# # get_overhang_to_lower(F_to::Type{<:YPFrequency}, F_from::Type{<:Union{Yearly{N1},Monthly{N2},Quarterly{N3}}}) where {N1, N2, N3} = _fconvert_start_overhang(F_to, F_from)
# get_overhang_to_lower(F_to::Type{<:Union{Yearly,Monthly,Quarterly}}, F_from::Type{<:Union{Yearly,Monthly,Quarterly}}) = _fconvert_start_overhang(F_to, F_from,check_parameters_from=false, check_parameters_to=false)
# get_overhang_to_lower(F_to::Type{<:Union{Yearly{N1},Quarterly{N1}}}, F_from::Type{<:Union{Yearly,Monthly,Quarterly}}) where N1 = _fconvert_start_overhang(F_to, F_from)
# get_overhang_to_lower(F_to::Type{<:Union{Yearly,Monthly,Quarterly}}, F_from::Type{<:Union{<:YPFrequency}}) = _fconvert_start_overhang(F_to, F_from, check_parameters_to=false)




# function _get_out_indices_old(F_to::Type{<:Union{Monthly,Quarterly{N1},Quarterly,Yearly{N2},Yearly,Weekly,Weekly{N3}}}, dates::Vector{Dates.Date}; check_parameter_to=false) where {N1,N2,N3}
#     months = Dates.month.(dates)
#     years = Dates.year.(dates)
...
# function _get_shift_to_higher2(F_to::Type{<:YPFrequency{N1}}, F_from::Type{<:YPFrequency{N2}}; values_base=:end, errors=true) where {N1,N2}
#     shift_length = 0
#     errors && _validate_fconvert_yp(F_to, F_from)
#     from_base_month = ppy_to_basemonth[ppy(F_from)]
#     to_base_month = ppy_to_basemonth[ppy(F_to)]
#     if hasproperty(F_from, :parameters) && length(F_from.parameters) > 0
#         from_base_month = F_from.parameters[1]
#     end
#     if hasproperty(F_to, :parameters) && length(F_to.parameters) > 0
#         to_base_month = F_to.parameters[1]
#     end

#     # i.e. to_bm = 7, fbm = 3,
#     # tbm - fbm = 4 

#     # MIT-out = ppy(F_to)*Int(MIT_from) / ppy(F_from)
#     # adjustment for off periods...
#     # input_adjustment
#     println(from_base_month - to_base_month, ", ", 12/ppy(F_to))
#     (d, r) = divrem(from_base_month - to_base_month, 12/ppy(F_to))
#     if d >= 1
#         println("a")
#         return to_base_month - r
#     else
#         println("b")
#         return to_base_month + r
#     end
