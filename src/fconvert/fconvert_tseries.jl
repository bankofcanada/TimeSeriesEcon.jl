...
# fconvert(F_to::Type{<:Union{Weekly,Weekly{N}}}, t::TSeries{<:Weekly}; method=:const, values_base=:end) where {N} = _fconvert_higher(F_to, t, method=method, values_base=values_base)



# to YP
# fconvert(F_to::Type{<:Union{Monthly,Quarterly{N1},Quarterly,Yearly{N2},Yearly},Weekly,Weekly{N3}}, t::TSeries{<:Weekly}; method=:mean, values_base=:end) where {N1,N2,N3} = _fconvert_lower(F_to, t, method=method, values_base=values_base)



# function fconvert(F_to::Type{<:BDaily}, t::TSeries{Daily})
#     fi = fconvert(F_to, firstdate(t))
...
# fconvert(F_to::Type{<:Union{Daily}}, t::TSeries{BDaily}; method=:const, values_base=:end) = _fconvert_higher(F_to, t, method=method, values_base=values_base)
...
#     rng_from = rangeof(t)
#     # if F_from == BDaily
#     #     padded_dates = [Dates.Date(val) for val in rng_from]
#     #     if get_option(:bdaily_skip_holidays)
#     #         holidays_map = get_option(:bdaily_holidays_map)
...
#     #     else
#     #         padded_dates = [Dates.Date(first(rng_from) - 1), padded_dates..., Dates.Date(last(rng_from) + 1)]
#     #     end
#     #     skip_nans = get_option(:bdaily_skip_nans)
#     # elseif F_from == Daily
#     #     padded_dates = collect(Dates.Date(first(rng_from)-1):Day(1):Dates.Date(last(rng_from) + 1))
#     # else
#     #     padded_dates = [Dates.Date(first(rng_from) - 1), [Dates.Date(val) for val in rng_from]..., Dates.Date(last(rng_from) + 1)]
#     # end
...
#     # if method == :point && values_base == :end
#     #     trunc_start = 0
#     # end
#     # # if the default and padded output periods are the same, then we do not have
...
#     #     trunc_end = 0
#     # end
#     # out_index = out_index[2:end-1]


#     # _full first_month if fi - 1 is an earlier month than fi
#     # full last month if li + 1 is a later month than fi


#     # println(out_index)
#     # trunc_start_data = trunc_start

#     # the issue here is that the first dates of the first data point is in 
#     # the previous month, so trunct_start_range = 1
#     # but the observational time of the first data point is actually in
#     # the current month, so trunc_start_data = 0

#     # if trunc_start == 1 && out_index[1] !== fconvert(F_to, t.firstdate, round_to=:begin)
#     #     trunc_start_data = 0
#     # end
#     # @show trunc_start, trunc_end
#     # println(method)
#     # fi = out_index[begin]
#     # li = out_index[end]
#     # method_for_trunc = method == :point ? values_base : method
#     # trunc_start, trunc_end = _get_fconvert_truncations(F_to, frequencyof(t), dates, method_for_trunc, pad_input=true)
...
#     # println(ret)
#     # @show fi, li, trunc_start, trunc_end
#     # @show fi2, li2, trunc_start_data, trunc_end_data
#     # trunc_start_data = trunc_start
#     # if fi2 > fi && trunc_start == 1
#     #     trunc_start_data = 0
#     # end
#     # trunc_end_data = trunc_end
#     # if li2 < li && trunc_end == 1
#     #     trunc_end_data = 0
#     # end
#     # trunc_start_data = fi2 > fi && trunc_start == 1 ? 1 : 0
#     # trunc_end_data = li2 < li && trunc_end == 1 ? 1 : 0
#     # @show trunc_start_data, trunc_end_data
