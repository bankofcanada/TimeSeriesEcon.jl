...
"""
fconvert(F_to::Type{<:Union{<:CalendarFrequency,<:YPFrequency}}, MIT_from::MIT{<:Union{<:CalendarFrequency,<:YPFrequency}}; ref = :end, round_to=:current)

Converts a MIT instance to the given target frequency.

`ref` determines the position within the input frequency to align with the output frequency. The options are `:begin`, `:end`. The default is `:end`.
...
For example, 
fconvert(Quarterly, 22Y, ref=:end) ==> 2022Q4
fconvert(Quarterly, 22Y, ref=:begin) ==> 2022Q1
"""

# MIT YP => YP
# having these different signatures significantly speeds up the performance; from a few microseconds to a few nanoseconds
fconvert(F_to::Type{<:Union{<:YPFrequency}}, MIT_from::MIT{<:Union{<:YPFrequency}}; ref=:end, round_to=:current) = _fconvert(sanitize_frequency(F_to), MIT_from, ref=ref, round_to=round_to)
function _fconvert(F_to::Type{<:Union{<:YPFrequency}}, MIT_from::MIT{<:Union{<:YPFrequency}}; ref=:end, round_to=:current)
    F_from = frequencyof(MIT_from)
    ref_adjust = ref == :end ? 1 : 0
    rounder = ref == :end ? ceil : floor
    from_month = Int(MIT_from+ref_adjust) * 12 / ppy(F_from) - ref_adjust
    from_month -= (12 / ppy(F_from)) - endperiod(F_from)
    out_period = (from_month + ref_adjust) / (12 / ppy(F_to)) - ref_adjust
    return MIT{F_to}(rounder(Integer, out_period))
end

# MIT Calendar => YP + Weekly
function fconvert(F_to::Type{<:Union{<:YPFrequency,<:Weekly}}, MIT_from::MIT{<:Union{Daily, BDaily, <:Weekly}}; ref=:end, round_to=:current)
    if ref == :end
...
# MIT YP => Weekly
function fconvert(F_to::Type{<:Weekly}, MIT_from::MIT{<:YPFrequency}; ref=:end, round_to=:current)
    if ref == :end
        return _get_out_indices(sanitize_frequency(F_to), [Dates.Date(MIT_from, :end)])[begin]
    elseif ref == :begin
        return _get_out_indices(sanitize_frequency(F_to), [Dates.Date(MIT_from, :begin)])[begin]
...
# MIT => BDaily
# function fconvert(F_to::Type{BDaily}, MIT_from::MIT{<:Union{<:Weekly,<:YPFrequency,Daily}}; ref=:end)
    
# end
function fconvert(F_to::Type{BDaily}, MIT_from::MIT{<:Union{<:Weekly,<:YPFrequency,Daily}}; ref=:end, round_to=:previous)
    if round_to == :previous
        return bdaily(Dates.Date(MIT_from, ref), bias=:previous)
    elseif round_to == :next
        return bdaily(Dates.Date(MIT_from, ref); bias=:next)
...
# MIT => Daily
fconvert(F_to::Type{Daily}, MIT_from::MIT{<:Union{<:Weekly,<:YPFrequency}}; ref=:end, round_to=:current) = daily(Dates.Date(MIT_from, ref))
function fconvert(F_to::Type{<:Daily}, MIT_from::MIT{BDaily}; ref=:end, round_to=:current)
    mod = Int(MIT_from) % 5
    if mod == 0
        mod = 5
    end
...
"""
fconvert_parts(F_to::Type{<:Union{<:YPFrequency}}, MIT_from::MIT{<:Union{<:YPFrequency}}; ref=:end, check_parameter_from=false, check_parameter_to=false)
...
"""
function fconvert_parts(F_to::Type{<:Union{<:YPFrequency}}, MIT_from::MIT{<:Union{<:YPFrequency}}; ref=:end)
    F_from = frequencyof(MIT_from)
...
function fconvert(F_to::Type{<:YPFrequency}, range_from::UnitRange{<:MIT{<:YPFrequency}}; trim=:both, parts=false)
    fi_to_period, fi_from_start_month, fi_to_start_month = fconvert_parts(sanitize_frequency(F_to), first(range_from), ref=:begin)
    li_to_period, li_from_end_month, li_to_end_month = fconvert_parts(sanitize_frequency(F_to), last(range_from), ref=:end)
...
function _fconvert_using_dates_parts(F_to::Type{<:Union{<:YPFrequency,<:Weekly}}, range_from::UnitRange{<:MIT{<:Union{<:CalendarFrequency}}}; trim=:both, errors=true, skip_all_nans::Bool=false, skip_holidays::Bool=false, holidays_map::Union{Nothing, TSeries{BDaily}} = nothing)
    ...
    if F_to > F_from # to higher frequency
        ...
        if trim !== :end && fconvert(F_from, fi, ref=:begin) != range_from[begin] 
            trunc_start = 1
        end
        ...
        if trim !== :begin && fconvert(F_from,li, ref=:end) != range_from[end]
            trunc_end = 1
        end
