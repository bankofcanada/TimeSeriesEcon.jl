...
### Functions:
 ...
 - Functions operating on `Series`
    ...
    - `apct(x::Series, islog::Bool = false)`: calculates annualized percent rate of change of `x::Series`
    - `nanrm!(x::Series, type::Symbol=:both)`: removes `NaN` from `x::Series`
"""
